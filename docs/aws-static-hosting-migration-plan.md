...
- **S3**: a private bucket ("Block all public access" on, versioning skipped), synced fresh on every deploy (`aws s3 sync --delete`). `aws s3 sync`'s MIME guessing doesn't recognize `.json`, so it uploads `manifest.json` as `application/octet-stream` — harmless for Chrome/Android but iOS Safari silently refuses to treat the page as an installable standalone PWA unless the manifest is served with a JSON-ish content type, so "Add to Home Screen" falls back to a bookmark tab with the address bar. A follow-up `aws s3 cp --content-type application/manifest+json --metadata-directive REPLACE` after the sync fixes this; Azure SWA had set it correctly on its own, so this only surfaced after the AWS cutover.
