...
- `findByOwnerAndDelegateAndStatus(String owner, String delegate, AccessStatus status)` — the authorization check (looks specifically for an `ACCEPTED` row for the pair; at most one can exist given the invariant above).
- `findByOwnerAndDelegateAndStatusIn(String owner, String delegate, List<AccessStatus> statuses)` — duplicate-invite guard, checked against `[PENDING, ACCEPTED]` before inserting a new invite.
...
New `DelegatedAccessFilter extends OncePerRequestFilter`, registered via `http.addFilterAfter(delegatedAccessFilter, JwtAuthenticationFilter.class)` in `SecurityConfig`.
