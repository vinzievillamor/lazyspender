...
```bash
./gradlew bootRunLocal                                      # run locally with spring.profiles.active=local (real, prod Firestore instance)
./gradlew build                                             # compile + run tests
./gradlew test                                               # run tests only
./gradlew test --tests "com.lazyspender.backend.BackendApplicationTests"  # single test class
```

`bootRunLocal` is just `bootRun --args='--spring.profiles.active=local'` wired up as its own Gradle task so it can be run with no args (see `build.gradle`).
